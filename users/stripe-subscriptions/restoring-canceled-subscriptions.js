...
module.exports = async (device) => {
  ...
  await user.clickLabel('After current billing period')
  ...
  await user.hover('Account')
  await user.click('Subscriptions & billing')
  await user.hover('Subscriptions')
  await user.screenshot('Click "Subscriptions"')
  await user.hover('Subscriptions')
  await user.click('Subscriptions')
  await user.hover(user.subscriptionid, true)
  await user.screenshot('Click subscription to restore')
  await user.hover(user.subscriptionid, true)
  await user.click(user.subscriptionid, true)
  await user.hover('Restore subscription')
  await user.screenshot('Click "Restore subscription"')
  await user.hover('Restore subscription')
  await user.click('Restore subscription')
  await user.hover('Restore subscription', true)
  await user.screenshot('Submit form')
  await user.hover('Restore subscription', true)
  await user.click('Restore subscription', true)
